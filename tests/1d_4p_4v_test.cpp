...
int main(int argc, char ** argv) {
    ...
    std::shared_ptr<dynamic::modeling::model> vol_0 = dynamic::translate::make_dynamic_atomic_model<volume_model_1d, TIME>(
        ...
        std::vector<particle_1d<TIME>>{
            {{0}, {1}, {0}, {1}, {1}, {0.01}, {0.05}, {0.03}, {0.9}}// the particle, inited in order like this [last_updated, id, species, mass, radius, [position], [velocity], [deferred_dv], deferred_dv_time]
        });
